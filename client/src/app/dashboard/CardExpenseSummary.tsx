...
const CardExpenseSummary = () => {
  ...
  const expenseSums = expenseByCategorySummary.reduce(
    (acc: ExpenseSums, item: ExpenseByCategorySummary) => {
      ...
      const amountUSD = parseInt(item.amount, 10);

      if (!acc[category]) acc[category] = 0;
      acc[category] += amountUSD;
  ...
  const expenseCategories = Object.entries(expenseSums).map(
    ([name, value]) => ({
      name,
      value, // still USD internally
    })
  );

  const totalExpensesUSD = expenseCategories.reduce(
            ...
            <div className="relative basis-3/5">
              ...
              <div className="absolute top-1/2 left-1/2 transform 
                ...
                <span className="font-bold text-xl">
                  {formatINRFromUSD(totalExpensesUSD)}
                </span>
              </div>
            </div>
                ...
                <div className="pt-2">
                  <p className="text-sm">
                    ...
                    <span className="font-semibold">
                      {formatINRFromUSD(expenseSummary.totalExpenses)}
                    </span>
                  </p>
                </div>
